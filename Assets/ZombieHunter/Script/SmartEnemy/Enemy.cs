...
public class Enemy : MonoBehaviour,ICanTakeDamage, IListener {
    ...
    [Range(0, 1)]
    public float soundShowUpVol = 0.7f;
    ...
    public void SetStartBehavior(STARTBEHAVIOR _state)
    {
        startBehavior = _state;
    }
    ...
    public void SpawnUp()
    {
        ...
        SetStartBehavior(STARTBEHAVIOR.BURROWUP);
    }
    ...
    public virtual void Die(){
        ...
        if (GetComponent<SpawnObjectOnDie>())
        {
            GetComponent<SpawnObjectOnDie>().Spawn();
        }
...
//	_2dxFX_Frozen[] iceFX;
    ...
    public void TakeDamage(float damage, Vector2 force, Vector2 hitPoint, GameObject instigator, BODYPART bodyPart = BODYPART.NONE, WeaponEffect weaponEffect = null, WEAPON_EFFECT forceEffect = WEAPON_EFFECT.NONE)
    {
        ...
        knockBackForce.x = Mathf.Abs(knockBackForce.x) * (transform.position.x > instigator.transform.position.x ? 1 : -1);
        if (hitFX)
