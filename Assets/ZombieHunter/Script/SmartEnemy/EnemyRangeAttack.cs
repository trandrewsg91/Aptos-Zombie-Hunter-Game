...
public class EnemyRangeAttack : MonoBehaviour {
    ...
    public void Shoot(bool isFacingRight){
		StartCoroutine (FireCo(isFacingRight));
    }
 ...
 //               projectile.Initialize (gameObject, _dir, Vector2.zero, false, damage*0.9f, damage*1.1f, 0);
    ...
    public IEnumerator FireCo(bool isFacingRight)
    {
        for (int i = 0; i < (dualShot ? 2 : 1); i++)
        {
            var _dir = Vector2.right * (isFacingRight ? 1 : -1);

            RaycastHit2D hit = Physics2D.Raycast(firePoint.position, _dir, 100, enemyLayer);
            ...
            if (hit)
            {
                ...
                if (takeDamage != null)
                {
                    takeDamage.TakeDamage(Random.Range(damage * 0.9f, damage), Vector2.zero, hit.point, gameObject);
                }
            }

            if (dualShot)
            {
               yield return new WaitForSeconds( fireSecondGunDelay);
            }
        }
